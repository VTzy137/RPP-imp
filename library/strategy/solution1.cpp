...
void solution1()
{
    ...
    for (int i = 0; i < 30; i++)
    {
        ...
        Point* tmp = new Point(Map::startPoint.x, Map::startPoint.y, nullptr);
        Point* tmp1 = new Point(Map::finishPoint.x, Map::finishPoint.y, nullptr);

        PSO::normalDirect[i] = init_population::initRandPath(tmp, tmp1);

        Path::population.push_back(Path(tmp));
    }
